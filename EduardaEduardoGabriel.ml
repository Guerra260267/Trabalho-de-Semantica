...
type value = Vnum of int 
           ...
           | Vrclos of variable * variable * expr * env (*AQUI: a mesma coisa*)
           | Vnil
...
let rec eval (environment : env) (e : expr) : value = 
  ...
  | Let(x, t, e1, e2) ->
    ...
    (match v1 with 
      | Vraise -> Vraise
      | _ -> eval (add_env_biding x v1 environment) e2
    )
  ...
  (* letrec *)
  | Lrec (f,t1,t2,x,e1,e2) ->
    let v1 = eval environment e2 in
      (
        match v1 with
          |  Vclos(x,e,envi) -> eval (add_env_biding f (Vrclos (f,x,e,environment)) envi) e2
          | _ -> failwith "unimplemented"
      )
